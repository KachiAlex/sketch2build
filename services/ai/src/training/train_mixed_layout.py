"""Mixed training script for the Layout Diffusion Model.

Trains on a blend of synthetic, RPlan, and CubiCasa5K data with
configurable sampling weights.

Usage:
    python -m src.training.train_mixed_layout \
        --mixed_dir data/mixed \
        --output_dir output/mixed_layout \
...
from src.training.train_layout import collate_fn, validate
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train Layout Diffusion on mixed data")
    parser.add_argument("--mixed_dir", type=str, required=True)
    parser.add_argument("--output_dir", type=str, default="./output/mixed_layout")
...
def train():
    ...
    logger.info("Starting mixed layout training", device=device, args=args)

    if settings.wandb_api_key:
        wandb.init(
            project=settings.wandb_project,
            entity=settings.wandb_entity,
            name="mixed_layout_diffusion",
            config=vars(args),
        )
    ...
    _, train_layout = create_mixed_datasets(
    ...
    _, val_layout = create_mixed_datasets(
        synthetic_dir=str(synthetic_dir) if synthetic_dir.exists() else "",
        rplan_dir=str(rplan_dir) if rplan_dir else None,
        cubicasa_dir=str(cubicasa_dir) if cubicasa_dir else None,
        split="val",
    ...
    logger.info(
        "Datasets loaded",
        train_layout=len(train_layout),
        val_layout=len(val_layout),
        weights=weights,
    )
